...
-- Results table - NO foreign key on section_id (CASCADE DELETE removed)
CREATE TABLE IF NOT EXISTS "results" (
    "id" UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    "run_id" UUID REFERENCES "runs"("id") ON DELETE CASCADE,
    "section_id" UUID NOT NULL,  -- Just a UUID, NOT a foreign key
    "value" JSONB,
    "metadata" JSONB,
    "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    "status" TEXT,
...
BEGIN
    ...
    new_version_entry := jsonb_build_object(
        ...
        'snapshot', jsonb_build_object(
            ...
            'sections', COALESCE(current_sections, '[]'::jsonb)
        ),
...
BEGIN
    ...
    BEGIN
        DELETE FROM sections WHERE template_id = p_template_id;
        ...
        FOR section_data IN SELECT * FROM jsonb_array_elements(snapshot->'sections')
        LOOP
            INSERT INTO sections (
                ...
                COALESCE(section_data->'metadata', '{}'::jsonb),
                COALESCE((section_data->>'created_at')::TIMESTAMP WITH TIME ZONE, NOW()),
                (section_data->>'created_by')::UUID,
                (section_data->>'updated_at')::TIMESTAMP WITH TIME ZONE,
                (section_data->>'updated_by')::UUID,
                COALESCE(section_data->'edit_history', '[]'::jsonb)
            );
        END LOOP;
...
BEGIN
    ...
    SELECT
        ...
        jsonb_array_length(vh->'snapshot'->'sections') as section_count
    FROM templates t,
