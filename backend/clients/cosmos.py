...
from core import Chunk, Match, File, Slice, Meta, Unit, Location
...
class CosmosVectorClient:
    ...
    async def search(
        ...
        try:
            ...
            matches = []
            for r in results:
                # Check if document has new format (units) or old format (text)
                if "units" in r and r["units"]:
                    # NEW FORMAT: unit-based storage
                    units = [Unit(**u) for u in r["units"]]
                    tokens = r.get("tokens", 0)
                else:
                    # OLD FORMAT: legacy text-based storage
                    # Convert old format to Unit objects for backwards compatibility
                    from core.types import Location

                    text = r.get("text", "")
                    # Create a single unit from the old text field
                    unit = Unit(
                        id=r["_id"],
                        type="text",
                        text=text,
                ...
                slice = (
                    Slice(sheet=r["sheet"], truncated=r["truncated"])
                    if r.get("sheet")
                    else None
                )
