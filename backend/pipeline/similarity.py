...
class SimpleNumberExtractor:
    ...
    def extract(self, text: str) -> List[Number]:
        ...
        for r in self.currency_model.parse(text):
            pos = (r.start, r.end)
            if pos not in seen:
                value = float(r.resolution.get('value', 0))
                unit = r.resolution.get('isoCurrency') or r.resolution.get('unit', 'USD')
        ...
        for r in self.percentage_model.parse(text):
            ...
            if pos not in seen:
                ...
                val_str = r.resolution.get('value', '0%')
...
class Similarity:
    ...
    def compute_similarity_scores(self, response_embedding: List[float],
                                  ...
                                  response_text: str,
                                  cited_texts: List[str]) -> List[Dict[str, float]]:
        ...
        for i, embedding in enumerate(cited_embeddings):
            ...
            # Always add boost for number matches
            try:
                match_count = self.number_extractor.count_matches(response_text, cited_texts[i])
                if match_count > 0:
                    # Add boost per match, cap at 1.0
                    similarity = min(1.0, similarity + (match_count * NUMBER_MATCH_BOOST))
            except Exception:
                # Silently skip number matching errors in production
                pass

            score_dict = {'score': float(similarity)}
