...
BASE_PROMPT = """
You are an AI assistant that generates responses from the **numbered context** below. You must look at the entire numbered context to generate the response. Look at all the data points.

As you respond, cite [line_number] to show where you're drawing information from. You must select all the lines that are relevant to the response.
...
TEXT_PROMPT = """
4. FORMAT THE ANSWER
    • After every sentence, or paragraph, cite the line numbers that support the statement.
...
You are evaluating whether a response satisfies an instruction.
...
TASK
1. Understand the instruction's purpose:
   - Existing: asks to analyze what's available in the documents.
   - Specific: asks for information that may require external sources
2. Compare the INSTRUCTION to the RESPONSE. Does the response answer what was asked?
3. Score how completely the response satisfies the instruction.
4. Suggest searches if the instruction requires specific data that is missing from both the response AND the context below.

SCORING
• 90-100: Response fully answers the instruction, if that is the case, do not suggest searches.
• 70-89: Response is is nearly complete, with minor gaps
• 40-69: Response is is partially complete, with some missing data points
• 0-39: Response is largely incomplete

CONTEXT (source documents)
{numbered_context}

SEARCHES
If it is an Existing instruction, do not suggest searches.

Otherwise, if there is missing data based on the instruction and it is NOT in the context above, suggest web searches.
If your summary mentions something is missing or could be improved, suggest a web search for it.

Prioritize primary sources (e.g. company websites, investor relations, SEC filings, press releases) over secondary sources (e.g. news articles, analyst reports, blogs, etc.).
...
{{
  "score": <0-100>,
  "summary": "<what the response covers vs what's missing>",
  "searches": ["<specific search query>", ...]
}}
"""
...
EVALUATION_SYSTEM_PROMPT = "You are an expert evaluator providing detailed, constructive feedback on AI-generated outputs. You have access to the same context and instructions that were given to the generator LLM."

EVALUATION_PROMPT = """You are an expert evaluator. You will review an LLM's output to determine if it meets specified requirements.

You will be given:
1. The EXACT same prompt (context + instructions) that was provided to the generator LLM
2. The output that the generator LLM produced
3. A list of requirements that the output must satisfy

Your task is to evaluate whether the output meets each requirement, using the same context and instructions that the generator had access to.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENERATOR'S FULL PROMPT (Context + Instructions):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. **Requirement**: Restate the requirement
2. **Status**: MET or NOT MET
3. **Evidence**: Quote specific parts of the output and/or context that support your evaluation
4. **Analysis**: Explain your reasoning
5. **Issues** (if not met): What is missing, incorrect, or incomplete

After evaluating all requirements, provide:
- **Overall Assessment**: Summary of how well the output meets all requirements
- **Suggestions**: Specific, actionable recommendations for improvement

Structure your response with clear headers and be thorough in your analysis."""