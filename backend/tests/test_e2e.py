...
6. Complete Field Processing Pipeline (TODO)
...
def test_context_building_and_token_budget():
    ...
    for file_idx in range(3):
        for chunk_idx in range(30):
            mock_chunks.append({
                ...
                "text": f"This is chunk {chunk_idx} from file {file_idx}. Revenue was ${(chunk_idx+1)*10}M in Q4 2024. " * 10,
    ...
    # Build context
    context = Context()
    context_text, line_map = context.build(mock_chunks, full_excel_map={})

    # Verify structure
    assert isinstance(context_text, str), "Context should be a string"
    assert len(context_text) > 0, "Context should not be empty"
    assert isinstance(line_map, dict), "line_map should be a dictionary"
...
async def test_citation_scoring_and_mapping():
    ...
    raw_response = "Revenue in Q4 2024 was $100 million [2], representing a 15% increase [3]."
    ...
    # Parse response
    response_items = citations.parse_response(raw_response, output_format=OutputFormat.TEXT)
    ...
    # Score citations (returns enriched line_map)
    final_line_map = await citations.score_response(
    ...
    # Verify scoring
    assert isinstance(final_line_map, dict), "Final line map should be a dictionary"

    print(f"\n✅ Test 5 PASSED: Citations scored and mapped")
    print(f"   Response items parsed: {len(response_items)}")
    print(f"   Final line map entries: {len(final_line_map)}")
    if final_line_map:
        first_key = list(final_line_map.keys())[0]
        print(f"   Sample line map entry: {first_key} -> {final_line_map[first_key].get('file_name')}")


# ==============================================================================
# Test 6: Complete Field Processing Pipeline
# ==============================================================================

@pytest.mark.asyncio
async def test_complete_field_processing_pipeline(client):
    """
