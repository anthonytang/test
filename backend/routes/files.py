...
from state import file_semaphore, state_manager
...
async def stream_file_processing(user_id: str, file_id: str, request: Request):
    ...
    try:
        ...
        await state_manager.update_progress("file", file_id, 0, "Downloading")
        yield {
            "event": "progress",
            "data": json.dumps(
        ...
        try:
            await state_manager.update_progress("file", file_id, 20, "Parsing")
            yield {
                "event": "progress",
                "data": json.dumps(
            ...
            await state_manager.update_progress("file", file_id, 45, "Analyzing")
            yield {
                "event": "progress",
                "data": json.dumps(
            ...
            await state_manager.update_progress("file", file_id, 65, "Indexing")
            yield {
                "event": "progress",
                "data": json.dumps(
            ...
            await state_manager.update_progress("file", file_id, 100, "Done")
            yield {
                "event": "completed",
                "data": json.dumps(
        ...
        # Final persistent state cleanup
        await state_manager.delete_job_state("file", file_id)
