...
async def stream_template_extraction(file_id: str, request: Request, token: str):
    ...
    await state_manager.set_job_state("extraction", generation_id, {"cancelled": False})

    async def stream_extraction_events():
        """Generator for SSE events during template extraction."""
        try:
            state = await state_manager.get_job_state("extraction", generation_id)
            ...
            try:
                ...
                async for event in extractor.generate_template_from_document(
                    tmp_file_path, template_name=None
                ):
                    # Check persistent state in every yield
                    state = await state_manager.get_job_state("extraction", generation_id)
                    if state and state.get("cancelled"):
                        yield {
                            "event": "cancelled",
            ...
            await state_manager.delete_job_state("extraction", generation_id)
...
async def abort_template_extraction(file_id: str, request: Request):
    ...
    cancelled_count = 0
    # List extraction IDs for this user/file pattern
    job_ids = await state_manager.list_job_ids("extraction", f"{user_id}_{file_id}_*")
    
    for gen_id in job_ids:
        state = await state_manager.get_job_state("extraction", gen_id)
        if state:
            state["cancelled"] = True
            await state_manager.set_job_state("extraction", gen_id, state)
            cancelled_count += 1
