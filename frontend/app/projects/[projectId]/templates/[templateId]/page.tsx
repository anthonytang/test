...
import { Field, ProjectWithPermissions } from "@studio/core";
import {
...
export default function ProjectTemplatePage() {
  ...
  const [isResizingCitations, setIsResizingCitations] = useState<
    "left" | false
  >(false);
  const [contextPanelWidth, setContextPanelWidth] = useState<number>(20);
  ...
  const [unsavedFieldsHighlight, setUnsavedFieldsHighlight] = useState<
    number[]
  >([]);
  ...
  const {
    ...
    setFieldErrors: setTemplateFieldErrors,
  ...
  const {
    ...
    currentProgress,
    fieldErrors: resultsFieldErrors,
    runs,
    selectedRun,
    setSelectedRun,
    setRuns,
  ...
  // Keep field errors separate - don't merge them
  // templateFieldErrors are for validation (empty names, etc)
  // resultsFieldErrors are for processing errors
  ...
  const displayFields =
    isReadOnly && currentRun?.metadata?.template_snapshot?.fields
      ? (currentRun.metadata.template_snapshot.fields as Field[])
      : fields;
  ...
  // Clear unsaved fields highlight after a delay
  useEffect(() => {
    if (unsavedFieldsHighlight.length > 0) {
      const timer = setTimeout(() => {
        setUnsavedFieldsHighlight([]);
      }, 3000);
  ...
  useEffect(() => {
    ...
    if (selectedTag || selectedSentence) {
      document.addEventListener("keydown", handleEscKey);
      return () => {
        document.removeEventListener("keydown", handleEscKey);
      };
    }
  }, [selectedTag, selectedSentence]);

  // Handle field reordering
  const reorderFields = async (reorderedFields: Field[]) => {
    try {
      // Update local state immediately for smooth UI
      setFields(reorderedFields);

      // Update each field's sort_order in the database
      const updatePromises = reorderedFields.map((field, index) =>
        azureApiClient.updateField(field.id, { sort_order: index })
      );
      ...
      if (template) {
        const fieldsData = await azureApiClient.getFields(template.id);
  ...
  const handleSaveResults = async () => {
    ...
    const exportFields = isReadOnly && displayFields ? displayFields : fields;
    const exportFiles =
    ...
    setIsSaving(true);
    try {
      // Capture chart images for chart-type fields
      const chartFieldIds = exportFields
        .filter((f: Field) => f.metadata?.type === "chart")
        .map((f: Field) => f.id);

      const chartImages = await captureAllChartImages(chartFieldIds);
  ...
  const handleExportWord = async () => {
    ...
    try {
      ...
      // Create a map of file IDs to names for quick lookup
      // Iterate through all lineMap values to capture file IDs from tables/charts too
      ...
      const fileNameMap = Object.fromEntries(
        Object.entries(fileInfoCache)
          ...
          .map(([id, info]: [string, any]) => [
            id,
            info.metadata?.source_type === "website"
              ? info.metadata?.source_url || info.name
              : info.name,
          ])
      );
      ...
      showSuccess(
        "Export Complete",
        "Results exported to Word successfully"
      );
    } catch (error) {
  ...
  // Wrapper for handleProcessTemplate that checks for unsaved fields
  const handleProcessTemplateWithValidation = async () => {
    // Check if there are any fields being edited
    if (editingFields.length > 0) {
      // Highlight unsaved fields
      setUnsavedFieldsHighlight(editingFields);

      // Scroll to the first unsaved field
      const firstUnsavedIndex = editingFields[0];
      const fieldElements = document.querySelectorAll("[data-field-index]");
      const unsavedFieldElement = fieldElements[firstUnsavedIndex];
      if (unsavedFieldElement) {
        unsavedFieldElement.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }
    ...
    // If all fields are saved, proceed with processing
        ...
        <div className="h-screen flex bg-white">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="max-w-4xl mx-auto relative">
                ...
                {showFilesPanel && (
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                      {isReadOnly && currentRun?.metadata?.available_files ? (
                        // Read-only view - show files from run metadata, similar to regular files but slightly different
                        <div className="space-y-0.5">
                          {currentRun.metadata.available_files.map(
                            (file: any, index: number) => (
                              <div
                                key={file.id || index}
                                className="group rounded px-3 py-2 bg-accent-50/30 border border-accent-100 cursor-default"
                              >
                                <div className="flex items-center gap-2">
                                  <svg
                                    className="h-3 w-3 text-accent-500 flex-shrink-0"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                  >
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  {(() => {
                                    const isWebsite =
                                      file.metadata?.source_type === "website";
                                    const displayName = isWebsite
                  ...
                  <div className="group relative flex items-start">
                    ...
                    >
                      <div className="relative">
                        ...
                        {/* File count badge */}
                        {files.length > 0 && (
                          <span className={`absolute -top-2 -right-2 min-w-[18px] h-[18px] flex items-center justify-center text-[10px] font-bold rounded-full ${
                ...
                {/* Questions Panel - Shows issues and gaps */}
                {Object.keys(results).length > 0 && (
                  <QuestionsPanel
                    ...
                    selectedRun={selectedRun}
                    isReadOnly={isReadOnly}
                    onFieldClick={(_fieldId) => {
                      // Just scroll to the field, don't open it for editing
                      // The scrolling is handled in the QuestionsPanel component itself
                    }}
                  />
                )}
                ...
                <FieldList
                  ...
                  editingFields={isReadOnly ? [] : editingFields}
                  fieldOperations={
                    isReadOnly
                      ? {
                          saving: [],
                  ...
                  fieldErrors={templateFieldErrors}
                  resultsErrors={resultsFieldErrors}
                  results={results}
                  ...
                  toggleFieldEdit={isReadOnly ? () => {} : toggleFieldEdit}
                  updateField={isReadOnly ? () => {} : updateField}
                  handleSaveField={
                    isReadOnly ? async () => {} : handleSaveField
                  }
                  removeField={isReadOnly ? async () => {} : removeField}
                  processSingleFieldWithRetry={
                    isReadOnly ? async () => {} : processSingleFieldWithRetry
                  }
                  handleAbortField={handleAbortField}
                  addField={isReadOnly ? () => {} : addField}
                  unsavedFieldsHighlight={unsavedFieldsHighlight}
          ...
          {selectedSentence && (
            <>
              ...
              >
                ...
                >
                  <CitedLines
                    tags={selectedSentence?.tags || []}
                    lineMap={
                      results[selectedSentence?.fieldId || ""]?.lineMap || {}
                    }
                    onTagSelect={(
                      tag: string | null,
                      lineNumbers: number[] | null
                    ) => {
                      if (tag === null || lineNumbers === null) {
                        setSelectedTag(null);
                      } else {
                        setSelectedTag({
                          fieldId: selectedSentence?.fieldId || "",
                          tag: tag,
                    ...
                    fieldName={
                      fields.find((f) => f.id === selectedSentence?.fieldId)
                        ?.name
                    }
                    fieldDescription={
                      fields.find((f) => f.id === selectedSentence?.fieldId)
                    ...
                    aiResponse={
                      results[selectedSentence?.fieldId || ""]?.text
                        ?.map((item) => item.line)
                        .join(" ") || ""
                    }
                  />
              ...
              >
                ...
                {selectedTag && (
                  <div className="flex-1 overflow-hidden">
                    <ContextViewer
                      lineMap={results[selectedTag.fieldId]?.lineMap || {}}
                      selectedLines={[selectedTag.tag]} // Pass the citation tag instead of parsed line numbers
                      fileInfoCache={fileInfoCache}
                      key={`${selectedTag.fieldId}-${selectedTag.tag}`}
                    />
                  </div>
                )}
              </div>
          ...
          <TemplateHistoryModal
            ...
            onRestore={async (_version) => {
              // Reload template and fields after restoration
