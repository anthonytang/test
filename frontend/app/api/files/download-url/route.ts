...
// -----------------------------
// Decode user info from Bearer token
// -----------------------------
function decodeUserFromToken(authHeader: string | null) {
  if (!authHeader?.startsWith("Bearer ")) {
    return { userEmail: "unknown", userName: "unknown" };
  }
  ...
  try {
    ...
    const userName =
      ...
      payload.displayName ||
      payload.given_name ||
      (userEmail !== "unknown" ? userEmail.split("@")[0] : "unknown") ||
      "unknown";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { userEmail, userName } = decodeUserFromToken(authHeader);
