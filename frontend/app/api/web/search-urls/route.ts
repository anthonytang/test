...
export async function POST(request: NextRequest) {
  ...
  if (isCloudMode) {
    ...
    const apimUrl = new URL(`${base}/studio/search-urls`);
    ...
    try {
      ...
      console.error("[SEARCH] Error proxying to APIM", err);
      return NextResponse.json(
        { error: "Error calling search-urls backend" },
        { status: 500 }
      );
    }
  }
  ...
  const backendUrl = `${BACKEND_URL.replace(/\/$/, "")}/search-urls`;
  ...
  try {
    ...
    if (!backendResponse.ok) {
      console.error(
        "[SEARCH] Backend returned non-OK",
    ...
    console.error("[SEARCH] Error calling backend", err);
    return NextResponse.json(
      { error: "Error calling search-urls backend" },
      { status: 500 }
    );
