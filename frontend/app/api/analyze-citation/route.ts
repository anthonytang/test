// app/api/analyze-citation/route.ts
...
export async function POST(request: NextRequest) {
  ...
  if (isCloudMode) {
    ...
    const apimUrl = new URL(`${base}/studio/analyze-citation`);
    ...
    try {
      ...
      if (!apimResponse.ok) {
        console.error(
          "[ANALYZE-CITATION] APIM returned non-OK",
          apimResponse.status,
      ...
      console.error("[ANALYZE-CITATION] Error proxying to APIM", err);
      return NextResponse.json(
        { error: "Error calling analyze-citation backend" },
        { status: 500 }
      );
    }
  }
  ...
  const backendUrl = `${BACKEND_URL.replace(/\/$/, "")}/analyze-citation`;
  ...
  try {
    ...
    if (!backendResponse.ok) {
      console.error(
        "[ANALYZE-CITATION] Backend returned non-OK",
    ...
    console.error("[ANALYZE-CITATION] Error calling backend", err);
    return NextResponse.json(
      { error: "Error calling analyze-citation backend" },
      { status: 500 }
    );
