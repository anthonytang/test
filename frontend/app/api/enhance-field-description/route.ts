// app/api/enhance-field-description/route.ts
...
export async function POST(request: NextRequest) {
  ...
  if (isCloudMode) {
    ...
    const apimUrl = new URL(`${base}/studio/enhance-field-description`);
    ...
    try {
      ...
      if (!apimResponse.ok) {
        ...
        try {
          ...
          return NextResponse.json(
            {
              error: "Failed to enhance field description",
      ...
      console.error("[ENHANCE-FIELD] Proxy error:", err);
      return NextResponse.json(
        { error: "Error calling backend enhance-field-description" },
        { status: 500 }
      );
    }
  }
  ...
  const backendUrl = `${BACKEND_URL.replace(/\/$/, "")}/enhance-field-description`;
  ...
  try {
    ...
    console.error("[ENHANCE-FIELD] Backend error:", err);
    return NextResponse.json(
      { error: "Error calling backend enhance-field-description" },
      { status: 500 }
    );
